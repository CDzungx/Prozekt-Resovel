<script>
   ...
   onMount(() => {
      ...
      const xScale = d3.scaleLinear([d3.min(rgdpData, (d) => d.year), d3.max(rgdpData, (d) => d.year)], [0, w]);
      const yScale = d3.scaleLinear([0, d3.max(rgdpData, (d) => Number(d.value.toFixed()) / 1000)], [h, 0]);
      ...
      svg.selectAll('rect')
         .data(rgdpData)
         .join('rect')
         .attr('class', 'fill-lime-500 hover:fill-green-800 opacity-90')
         ...
         .on('mouseover', (i, d) => {
            d3.select('#tooltip').attr('data-date', d.year).attr('data-gdp', d.value).classed('hidden', false);
            d3.select('#tooltip').select('#year').text(`${$GDPContent.year}: ${d.year}`);
            d3.select('#tooltip')
               .select('#gdpValue')
               .text(`GDP: ${Number(d.value.toFixed(2)).toLocaleString()} ${$GDPContent.billion}`);
         })
         .on('mouseout', () => {
...
<div class="flex h-screen">
   <div
      ...
      <div
         id="tooltip"
         class="absolute ml-14 mt-2 hidden h-fit w-fit rounded-md border-2 border-gray-500 bg-white bg-opacity-75 p-2 px-2 font-mono text-sm font-bold md:ml-14 md:px-3 md:text-base lg:ml-16 lg:px-4 lg:text-lg">
         <div id="year"></div>
         <div id="gdpValue"></div>
      </div>
      ...
      <div id="srcInfo" class="mt-2 flex items-end justify-between text-sm">
         <span class="text-left text-xs text-yellow-900">VNGDP - V1.8</span>
